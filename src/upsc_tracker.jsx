...
import {
  ...
  updateLastSection, checkIsSuperAdmin, deleteFeedPost,
} from './supabaseData';
import {
...
function CommunityPanel() {
  ...
  const handleDeletePost = async (summaryId) => {
    if (!window.confirm("Are you sure you want to delete this post as Super Admin?")) return;
    const res = await deleteFeedPost(summaryId);
    if (!res.error) {
      await loadFeed();
    } else {
  ...
  useEffect(() => {
    loadFeed();
    loadLeaderboard();
    checkIsSuperAdmin(userId).then(setIsSuperAdmin);
      ...
      <div className="lg:col-span-1 space-y-4">
        <div className="bg-white dark:bg-slate-800/60 rounded-2xl border border-slate-200 dark:border-slate-700/40 p-5">
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 mb-4">
            Ranks public users by total study hours logged over the last 30 days. Study more daily to top the leaderboard. 
          </p>
                  ...
                  >
                    <div className="flex items-center justify-between gap-3 mb-2.5">
                      ...
                      <div className="text-right flex items-center gap-2">
                        {isSuperAdmin && (
                          <button
                            onClick={() => handleDeletePost(act.id)}
                            className="text-[10px] text-rose-500 hover:text-rose-600 bg-rose-500/10 hover:bg-rose-500/20 px-2 py-1 rounded transition-colors"
